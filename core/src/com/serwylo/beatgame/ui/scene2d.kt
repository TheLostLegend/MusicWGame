...
fun makecheck(label: String, skin: Skin): CheckBox{
    val myCheckbox = CheckBox(label, skin)
    myCheckbox.setSize(100f, 100f) // Устанавливаем размеры Checkbox
    return myCheckbox
}
...
fun makeHeading(title: String, icon: TextureRegion, styles: Assets.Styles, strings: I18NBundle, onBack: (() -> Unit)? = null): HorizontalGroup {
    return HorizontalGroup().apply {
        ...
        if (onBack != null) {
            addActor(makeButton(strings["btn.back"], styles, onBack))
        }
    }
}
