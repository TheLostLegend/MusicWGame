...
class Player(
    ...
    private val walkAnimation: Animation<TextureRegion> = Animation(0.8f, sprites.character_a_walk)
    ...
    private var health = health * 20
    private var shield = armor2 * 20
    ...
    private val getObstacles = mutableSetOf<Obstacle>()
...
//        }
        ...
        if (score.getMultiplier() > MIN_MULTIPLIER_FOR_RAINBOW) {
            jumpParticles.emitters.forEach {
                it.setPosition(position.x + WIDTH*5/8, position.y)
            }
...
//        }
//    }
    fun get(obstacle: Obstacle) {
        if (!getObstacles.contains(obstacle)) {
            ...
            if (score.getMultiplier() >= MIN_MULTIPLIER_FOR_RAINBOW) {
                jumpParticles.emitters.forEach {
                    it.emission.highMin = (score.getMultiplier() - MIN_MULTIPLIER_FOR_RAINBOW) * 10
                    it.emission.highMax = (score.getMultiplier() - MIN_MULTIPLIER_FOR_RAINBOW) * 15
                    it.duration = (score.getMultiplier() - MIN_MULTIPLIER_FOR_RAINBOW) * 15 * 10
                    it.maxParticleCount = ((score.getMultiplier() - MIN_MULTIPLIER_FOR_RAINBOW) * 25).toInt()
                }
                jumpParticles.start()
                val increaseJumpPower = ((score.getMultiplier() - MIN_MULTIPLIER_FOR_RAINBOW) * SHIELD_EARNED_PER_JUMP).coerceAtMost(SHIELD_MAX_INCREASE).toInt()
    ...
    fun hit(obstacle: Obstacle) {
        ...
        if (!hitObstacles.contains(obstacle)) {
            ...
            val damage = 20
                //(obstacle.rect.area() * AREA_TO_DAMAGE).toInt().coerceAtLeast(MIN_DAMAGE)
...
//            }
            ...
            shield -= damage
            if (shield < 0) {
                // Any left over damage that the shield couldn't absorb is applied back to the health.
                val nonShieldDamage = -shield
                health -= nonShieldDamage
                shield = 0
            }

            justHitDamage = damage
    ...
    companion object {
        ...
        const val HIT_ANIMATION_DURATION = 0.4f
