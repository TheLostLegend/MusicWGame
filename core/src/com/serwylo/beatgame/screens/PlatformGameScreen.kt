...
class PlatformGameScreen(
    ...
    private val camera = makeCamera(5)
    private lateinit var hud: HUD
    private val obstacles = mutableListOf<Obstacle>()
    private val bonus = mutableListOf<Obstacle>()
    private val collectedBonus = mutableListOf<Obstacle>()
    ...
    fun makeCamera(maxWidth:Int): ParallaxCamera {
        ...
        if (Gdx.graphics.width > Gdx.graphics.height) {
            ...
            width = maxWidth.toFloat()
            height = maxWidth * aspectRatio
//            width = maxHeight * aspectRatio/scaleFactor
//            height = maxHeight.toFloat()/scaleFactor
        }

        var camera = ParallaxCamera(width, height)
        return camera
    }
    ...
    override fun show() {
        ...
        hud = HUD(
            ...
            Ships.byShip(selectedShip).health,
            Ships.byShip(selectedShip).armor
        ...
        Gdx.input.inputProcessor = InputMultiplexer(stage, hud.getInputProcessor(), object : InputAdapter() {
            ...
            override fun keyDown(keycode: Int): Boolean {
                if (keycode == Input.Keys.SPACE || keycode == Input.Keys.LEFT || keycode == Input.Keys.RIGHT) {
                    if (state == State.PAUSED) {
                        resume()
                    }
        ...
        camera.translate(camera.viewportWidth/10, camera.viewportHeight/2 - screenToWorld(hud.bottomGutterHeightInPixels()) - TiledSprite.TILE_SIZE, 0f)
        camera.update()

        player = Player(score, Vector2(-0.4f, SCALE_X), sprites, game.assets.getParticles(), Ships.byShip(selectedShip).health, Ships.byShip(selectedShip).armor, width)
...
//        camera.update()
    ...
    override fun render(delta: Float) {
        ...
        if (state == State.DYING || state == State.WINNING || state == State.PAUSED) {
            ...
            shapeRenderer.rect(0f, 0.0f, Gdx.graphics.width.toFloat(), Gdx.graphics.height.toFloat())
            shapeRenderer.end()
        }
    ...
    private fun processInput() {
        if (Gdx.input.isTouched || Gdx.input.isKeyJustPressed(Input.Keys.SPACE) || Gdx.input.isKeyJustPressed(Input.Keys.LEFT) || Gdx.input.isKeyJustPressed(Input.Keys.RIGHT)) {
            if (state == State.PENDING) {
                state = State.WARMING_UP
                startTime = Globals.animationTimer
            } else if (state == State.PLAYING || state == State.WARMING_UP) {
                if (Gdx.input.isKeyJustPressed(Input.Keys.LEFT) ){
                    player.position.x -= 8 * Gdx.graphics.deltaTime }
                    ...
                    player.position.x=touchPos.x - 0.5f
                }
            }
        }
    }
    ...
    private fun updateEntities(delta: Float) {
        ...
        if (state == State.PLAYING || state == State.WARMING_UP) {
            ...
            if (player.position.y >= (world.duration + WARM_UP_TIME + END_LEVEL_WALK_TIME) * SCALE_X && state != State.WINNING) {
    ...
    private fun scrollCamera(camera: OrthographicCamera, delta: Float) {
        camera.translate(0f, delta * SCALE_X)
        camera.update()
    }
    ...
    private fun renderEntities() {
        ...
        for (i in (leftMostObstacleOnScreenIndex - 20).coerceAtLeast(0) until bonus.size) {
            val obstacle = bonus[i]
            if (obstacle.rect.x > cameraRight || collectedBonus.contains(obstacle)) {
                continue
            }
    ...
    override fun pause() {
        ...
        val pauseGameInfo = PauseGameActor(
            game,
            { resume() },
            { leaveGame { game.startGame(world , selectedShip) } },
    ...
    private fun checkCollisions() {
        ...
        for (i in 0 until bonus.size) {
            val obstacle = bonus[i]
            if (player.isColliding(obstacle.rect)) {
                player.get(obstacle)
                collectedBonus.add(obstacle)
            }
        }
    }
    ...
    private fun generateObstacles(sprites: Assets.Sprites, features: List<Feature>): List<Obstacle> {
        ...
        var maxSt = features.maxOf { it.strength }
        var pos: Int

        val bonuses: MutableList<Rectangle> = mutableListOf()
        val rects: MutableList<Rectangle> = features.sortedBy { it.startTimeInSeconds }.map {
...
//            val rawWidth = (it.strength * Obstacle.STRENGTH_TO_WIDTH).coerceAtLeast(Obstacle.MIN_WIDTH)
//            val roundedWidth = ObstacleBuilder.roundToNearestTile(rawWidth)
//
//            val rawHeight = it.durationInSeconds * SCALE_X
...
//            val isSingleTile = rawHeight < ObstacleBuilder.TILE_SIZE && rawWidth < ObstacleBuilder.TILE_SIZE
            ...
            if (it.strength < maxSt/5*2) pos = 1
            else if (it.strength > maxSt/5*3) pos = 3
            else pos = 2
            pos =(pos + it.startTimeInSeconds).toInt()%3+1
            Rectangle(
                (pos * 0.125f + (pos-1)*1.5f) - 2f,
...
//
//                    val distanceToNext = next.y - current.y - current.height
...
//                        current.height += distanceToNext + next.height
//                        toRemoveIndices.add(nextIndex)
//
//                    } else if (current.height <= ObstacleBuilder.TILE_SIZE && next.height <= ObstacleBuilder.TILE_SIZE) {
...
//            i ++
//        }
        bonus.addAll(bonuses.map { ObstacleBuilder.makeObstacle(it, sprites, true) })
        return rects.map { ObstacleBuilder.makeObstacle(it, sprites, false) }

    }

    private fun endGame() {
        music.volume = 0.4f
        score.setPoints(score.getPoints() * Ships.byShip(selectedShip).boost/100)
