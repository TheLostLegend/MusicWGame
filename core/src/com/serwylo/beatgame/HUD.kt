...
class HUD(
    ...
    private val hearts = DiscreteProgressBar(makeIconDrawable(sprites.heart), makeIconDrawable(sprites.heart_half), makeIconDrawable(sprites.heart_empty), particles.health, true, health)
    private val shields = DiscreteProgressBar(makeIconDrawable(sprites.shield_full), makeIconDrawable(sprites.shield_half), makeIconDrawable(sprites.shield_empty), particles.shield, false, armor)
...
class DiscreteProgressBar(
    ...
    private val images = (1..point).map { Image(textureFull) }
    var value = point * 20f
        set(newValue) {
